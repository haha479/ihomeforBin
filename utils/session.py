...
class Session(object):
	def __init__(self, request_handler_obj):
		...
		self.session_id = reuest_handler_obj.get_secure_cookie('session_id')
		
		#如果不存在session_id, 生成session_id		
		if not self.session_id:
			self.session_id = uuid.uuid4().hex
			self.data = {}
			request_hander_obj.set_secure_cookie('session_id')

		#但如果存在session_id, 去redis中取出data
		else:
			try:
				json_data = request_hander_obj.redis.get('sess_%s'%self.session_id)	
